	...
	for element := pollTiFlashContext.UpdatingProgressTables.Front(); element != nil && pollMaxCount > 0; pollMaxCount-- {
		...
		if checkTiFlash {
			if len(availableTableID.ShardIDs) > 0 {
				tiflashProgress, _, err = infosync.CalculateTiFlashProgressForShards(availableTableID.ShardIDs, tableInfo.TiFlashReplica.Count, pollTiFlashContext.TiFlashStores)
			} else {
		...
		if !available && !tb.LogicalTableAvailable {
			...
			if checkTiFlash {
				...
				if len(tb.ShardIDs) > 0 {
					tiflashProgress, tiflashAvailProgress, err = infosync.CalculateTiFlashProgressForShards(tb.ShardIDs, tb.Count, pollTiFlashContext.TiFlashStores)
				} else {
