...
func PollAvailableTableProgress(schemas infoschema.InfoSchema, _ sessionctx.Context, pollTiFlashContext *TiFlashManagementContext) {
	...
	for element := pollTiFlashContext.UpdatingProgressTables.Front(); element != nil && pollMaxCount > 0; pollMaxCount-- {
		...
		if len(availableTableID.ShardIDs) > 0 {
			progress, err = infosync.CalculateTiFlashProgressForShards(availableTableID.ShardIDs, tableInfo.TiFlashReplica.Count, pollTiFlashContext.TiFlashStores)
		} else {
	...
	for _, tb := range tableList {
		...
		if !available && !tb.LogicalTableAvailable {
			...
			if len(tb.ShardIDs) > 0 {
				progress, err = infosync.CalculateTiFlashProgressForShards(tb.ShardIDs, tb.Count, pollTiFlashContext.TiFlashStores)
			} else {
