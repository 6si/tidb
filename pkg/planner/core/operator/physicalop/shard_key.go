...
// ExtractTableInfoFromPlan traverses down the plan tree to find the underlying
// PhysicalTableScan or PhysicalIndexScan and returns its TableInfo.
func ExtractTableInfoFromPlan(plan base.PhysicalPlan) *model.TableInfo {
	for plan != nil {
		switch p := plan.(type) {
...
// ShardKeyColumnsMatch checks whether all the given MPP partition columns are covered by
// the shard key columns of the table.
